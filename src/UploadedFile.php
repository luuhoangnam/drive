 ...
 *
 * @method UploadedFile backup(string $name = 'default')
 *                                              Backups current image state as fallback for reset method under an
 *                                              optional name. Overwrites older state on every call, unless a different
 *                                              name is passed.
 * @method UploadedFile blur(integer $amount = 1)
 *                                              Apply a gaussian blur filter with a optional amount on the current
 *                                              image. Use values between 0 and 100.
 * @method UploadedFile brightness(integer $level)
 *                                              Changes the brightness of the current image by the given level. Use
 *                                              values between -100 for min. brightness. 0 for no change and +100 for
 *                                              max. brightness.
 * @method UploadedFile cache(\Closure $callback, integer $lifetime = null, boolean $returnObj)
 *                                              Method to create a new cached image instance from a Closure callback.
 *                                              Pass a lifetime in minutes for the callback and decide whether you want
 *                                              to get an Intervention Image instance as return value or just receive
 *                                              the image stream.
 * @method UploadedFile canvas(integer $width, integer $height, mixed $bgcolor = null)
 *                                              Factory method to create a new empty image instance with given width
 *                                              and height. You can define a background-color optionally. By default
 *                                              the canvas background is transparent.
 * @method UploadedFile circle(integer $radius, integer $x, integer $y, \Closure $callback = null)
 ...
 * @method UploadedFile colorize(integer $red, integer $green, integer $blue)
 ...
 * @method UploadedFile contrast(integer $level)
 *                                              Changes the contrast of the current image by the given level. Use
 *                                              values between -100 for min. contrast 0 for no change and +100 for max.
 *                                              contrast.
 * @method UploadedFile crop(integer $width, integer $height, integer $x = null, integer $y = null)
 ...
 * @method UploadedFile destroy()
 *                                              Frees memory associated with the current image instance before the PHP
 *                                              script ends. Normally resources are destroyed automatically after the
 *                                              script is finished.
 * @method UploadedFile ellipse(integer $width, integer $height, integer $x, integer $y, \Closure $callback = null)
 ...
 *                                              callback.
 * @method UploadedFile exif(string $key = null)
 *                                              Read Exif meta data from current image.
 * @method UploadedFile iptc(string $key = null)
 *                                              Read Iptc meta data from current image.
 * @method UploadedFile fill(mixed $filling, integer $x = null, integer $y = null)
 *                                              Fill current image with given color or another image used as tile for
 *                                              filling. Pass optional x, y coordinates to start at a certain point.
 ...
 * @method UploadedFile heighten(integer $height, \Closure $callback = null)
 ...
 * @method UploadedFile insert(mixed $source, string $position = 'top-left', integer $x = 0, integer $y = 0)
 ...
 *                                              maintained.
 * @method UploadedFile interlace(boolean $interlace = true)
 *                                              Determine whether an image should be encoded in interlaced or standard
 *                                              mode by toggling interlace mode with a boolean parameter. If an JPEG
 *                                              image is set interlaced the image will be processed as a progressive
 *                                              JPEG.
 * @method UploadedFile invert()
 *                                              Reverses all colors of the current image.
 * @method UploadedFile limitColors(integer $count, mixed $matte = null)
 *                                              Method converts the existing colors of the current image into a color
 *                                              table with a given maximum count of colors. The function preserves as
 *                                              much alpha channel information as possible and blends transarent pixels
 *                                              against a optional matte color.
 * @method UploadedFile line(integer $x1, integer $y1, integer $x2, integer $y2, \Closure $callback = null)
 ...
 * @method UploadedFile make(mixed $source)
 *                                              Universal factory method to create a new image instance from source,
 *                                              which can be a filepath, a GD image resource, an Imagick object or a
 *                                              binary image data.
 * @method UploadedFile mask(mixed $source, boolean $mask_with_alpha)
 *                                              Apply a given image source as alpha mask to the current image to change
 *                                              current opacity. Mask will be resized to the current image size. By
 *                                              default a greyscale version of the mask is converted to alpha values,
 *                                              but you can set mask_with_alpha to apply the actual alpha channel. Any
 *                                              transparency values of the current image will be maintained.
 * @method UploadedFile opacity(integer $transparency)
 *                                              Set the opacity in percent of the current image ranging from 100% for
 *                                              opaque and 0% for full transparency.
 * @method UploadedFile orientate()
 *                                              This method reads the EXIF image profile setting 'Orientation' and
 *                                              performs a rotation on the image to display the image correctly.
 * @method UploadedFile pickColor(integer $x, integer $y, string $format = 'array')
 *                                              Pick a color at point x, y out of current image and return in optional
 *                                              given format.
 * @method UploadedFile pixel(mixed $color, integer $x, integer $y)
 *                                              Draw a single pixel in given color on x, y position.
 * @method UploadedFile pixelate(integer $size)
 *                                              Applies a pixelation effect to the current image with a given size of
 *                                              pixels.
 * @method UploadedFile polygon(array $points, \Closure $callback = null)
 *                                              Draw a colored polygon with given points. You can define the appearance
 *                                              of the polygon by an optional closure callback.
 * @method UploadedFile rectangle(integer $x1, integer $y1, integer $x2, integer $y2, \Closure $callback = null)
 ...
 *                                              parameter.
 * @method UploadedFile reset(string $name = 'default')
 *                                              Resets all of the modifications to a state saved previously by backup
 *                                              under an optional name.
 * @method UploadedFile resize(integer $width, integer $height, \Closure $callback = null)
 ...
 *                                              Rotate the current image counter-clockwise by a given angle. Optionally
 *                                              define a background color for the uncovered zone after the rotation.
 * @method UploadedFile sharpen(integer $amount = 10)
 *                                              Sharpen current image with an optional amount. Use values between 0 and
 *                                              100.
 * @method UploadedFile text(string $text, integer $x = 0, integer $y = 0, \Closure $callback = null)
...
class UploadedFile
{
    ...
    private function makeTemporaryFile($original = true)
    {
        ...
        $temp = $this->config->get('drive.location.temporary');
        $name = Str::random();
    ...
    {
        $directory = $this->config->get('drive.location.temporary');
    ...
    {
        $location  = $this->config->get('drive.location.storage');
